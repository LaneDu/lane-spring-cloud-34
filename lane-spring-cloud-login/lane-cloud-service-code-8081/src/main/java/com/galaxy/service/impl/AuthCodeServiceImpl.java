...
import com.galaxy.api.SendEmailFeignClient;
import com.galaxy.dao.AuthCodeDao;
import com.galaxy.pojo.AuthCodePojo;
import com.galaxy.service.AuthCodeService;
...
//    private Random random = new Random();
    ...
    public boolean sendCode(String email)  {
        ...
        try {
            //发送验证码
            String sendEmail = sendEmailFeignClient.sendEmail(email, code);
            //保存验证码
            saveCode(email, code);
        } catch (Exception e) {
            String format = String.format("fail to send email or save code :%s" + email);
